...
public enum smallState//定义枚举类型，保存野怪的五种状态
{
    Idle,//静立状态
    Walk,//行走状态
    Attack,//攻击状态
...
public class small_walk : MonoBehaviour
{
    public smallState state;//状态量
    public string animall_idle;//静立动画
    public string animall_walk;//行走动画
    public string animall_attack;//攻击动画
    ...
    void Start()
    {
        ...
        player = GameObject.Find("DARKNESS_WARLORD");
        target = player.transform;//获取玩家的位置
        animation = GetComponent<Animation>();
        render = GetComponent<Renderer>();
        gb = GetComponent<gebulin>();
    }
...
{
    state = smallState.Attack;
}
if (state == smallState.Attack)
{
    AutoAttack();//野怪执行攻击
}
else if (state == smallState.Death)//如果野怪的状态是死亡，则播放死亡动画
{
...
{
    if (state == smallState.Death) return;
    ...
    {
        this.hp -= attack;
        Slider_c1.instance.SetValue(hp);
        StartCoroutine(ShowBodyRed());
        if (hp <= 0)
        {
            state = smallState.Death;
